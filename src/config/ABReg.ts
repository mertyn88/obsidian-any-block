...
export const ABReg = {
   ...
   * 
   * 주의:
   * - (?!\[) (?!\toc) 이런 후방 부정 문장은 하나의 매칭 항목으로 간주되지 않습니다.
   * - `%%`와 `:`를 허용하는 규칙은 V3에 새로 추가되었습니다.
   */
  // 접두사 버전 (선택자용)
  reg_header:   /^((\s|>\s|-\s|\*\s|\+\s)*)(%%)?(\[((?!toc|TOC|< )[\|#0-9a-zA-Z\u4e00-\u9fa5].*)\]):?(%%)?\s*$/, // 빈 `|`로 첫 문자 제한을 해제할 수 있습니다. (`|` 주의: 엄격 모드로 사용할 수 있습니다, `#` 주의: 뒤에 한 칸 띄워서 "태그"로 변하지 않도록 권장, `!` 주의: `> [!note]`로 잘못 트리거되지 않도록 주의
